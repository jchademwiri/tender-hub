  ...
  try {
    ...
    if (publishers.length > 0) {
      ...
      for (const publisher of publishers) {
        ...
        const province = await sql`SELECT * FROM provinces WHERE id = ${publisher.province_id}`;

        if (province.length === 0) {
          console.log(`❌ Publisher "${publisher.name}" has invalid province_id: ${publisher.province_id}`);

          // Find a valid province to assign (let's use Gauteng as default)
          const validProvince = await sql`SELECT * FROM provinces WHERE code = 'GP' LIMIT 1`;
          ...
          console.log(`✅ Publisher "${publisher.name}" -> Province: ${province[0].name}`);
        }
      }
    } else {
