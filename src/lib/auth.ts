    ...
    deleteUser: {
      ...
      beforeDelete: async (user, _request) => {
        ...
        await db
          ...
          .where(eq(schema.pageViews.userId, user.id));
        await db
          .delete(schema.userInteractions)
          .where(eq(schema.userInteractions.userId, user.id));
        await db.delete(schema.events).where(eq(schema.events.userId, user.id));
        await db
          .delete(schema.userConsent)
          .where(eq(schema.userConsent.userId, user.id));
      },
