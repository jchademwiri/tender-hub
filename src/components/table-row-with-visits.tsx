...
export function TableRowWithVisits({
  ...
  const formatLastVisit = React.useCallback((date: Date): string => {
    ...
    if (diffInSeconds < 3600)
      return `Visited ${Math.floor(diffInSeconds / 60)} minutes ago`;
    if (diffInSeconds < 86400)
      return `Visited ${Math.floor(diffInSeconds / 3600)} hours ago`;
  ...
  return (
    <Tooltip>
          ...
          {columns.map((col) => (
            <TableCell key={col.key}>
              <div className="flex items-center gap-2">
                {col.render
                  ? col.render((publisher as any)[col.key], publisher)
                  : String((publisher as any)[col.key])}
      ...
      <TooltipContent side="top" className="text-xs max-w-xs">
        <div className="whitespace-pre-line">{rowTooltipContent}</div>
      </TooltipContent>
    </Tooltip>
  );
}
