...
async function getInvitationAnalytics() {
  try {
    ...
    return {
      ...
      // Status message
      status: "Analytics system is initializing. Full data will be available once invitations are created."
    };
...
async function exportAnalyticsData(analyticsData: any, format: string) {
  try {
    ...
    return NextResponse.json({ error: 'Export format not supported' }, { status: 400 });
...
async function exportToCSV(analyticsData: any, timestamp: string) {
  const sections = [
    ...
    ['Period', `${analyticsData.period?.from || 'All Time'} to ${analyticsData.period?.to || 'Now'}`],
    [''],

    // Key metrics
    ['KEY METRICS'],
    ['Metric', 'Value', 'Description'],
    ['Total Invitations', analyticsData.totalInvitations, 'All invitations sent'],
    ['Pending Invitations', analyticsData.pendingInvitations, 'Awaiting response'],
    ['Accepted Invitations', analyticsData.acceptedInvitations, 'Successfully accepted'],
    ['Expired Invitations', analyticsData.expiredInvitations, 'Past expiry date'],
    ['Cancelled Invitations', analyticsData.cancelledInvitations, 'Manually cancelled'],
    ['Conversion Rate (%)', analyticsData.conversionRate, 'Acceptance rate'],
    ['Average Response Time (hours)', analyticsData.averageResponseTime, 'Time to accept'],
    [''],

    // Role-based performance
    ['PERFORMANCE BY ROLE'],
    ['Role', 'Sent', 'Accepted', 'Success Rate (%)'],
    ['Admin', analyticsData.byRole.admin.sent, analyticsData.byRole.admin.accepted, analyticsData.byRole.admin.rate],
    ['Manager', analyticsData.byRole.manager.sent, analyticsData.byRole.manager.accepted, analyticsData.byRole.manager.rate],
    ['User', analyticsData.byRole.user.sent, analyticsData.byRole.user.accepted, analyticsData.byRole.user.rate],
    [''],

    // Daily trends
    ...(analyticsData.trends?.length ? [[
      'DAILY TRENDS',
      'Date', 'Total Sent', 'Accepted', 'Pending', 'Expired', 'Conversion Rate (%)'
    ], ...analyticsData.trends.map((trend: any) => [
      '', trend.date, trend.total, trend.accepted, trend.pending, trend.expired, trend.conversionRate
    ]), ['']] : []),

    // Weekly performance
    ...(analyticsData.weeklyStats?.length ? [[
      'WEEKLY PERFORMANCE',
      'Week', 'Total Sent', 'Accepted', 'Conversion Rate (%)'
    ], ...analyticsData.weeklyStats.map((week: any) => [
      '', week.week, week.total, week.accepted, week.conversionRate
    ]), ['']] : []),

    // Recent events
    ...(analyticsData.events?.length ? [[
      'RECENT EVENTS',
      'Event Type', 'Event Name', 'Count'
    ], ...analyticsData.events.map((event: any) => [
      '', event.type, event.name, event.count
    ]), ['']] : [])
  ];

  const csvContent = sections
    .map(section => section.map((row: any) =>
      Array.isArray(row) ? row.map((cell: any) => `"${cell}"`).join(',') : `"${row}"`
    ).join('\n'))
    .join('\n\n');
