...
export default function ManagerPublishersPage() {
  ...
  const fetchProvinces = async () => {
    try {
      const response = await fetch("/api/manager/provinces?limit=1000");
      if (response.ok) {
  ...
  const fetchPublishers = async (search = "", page = 1) => {
    try {
      ...
      const response = await fetch(`/api/manager/publishers?${params}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleUpdatePublisher = async (prevState: any, formData: FormData) => {
    ...
    try {
      const response = await fetch(`/api/manager/publishers/${editingPublisher.id}`, {
        method: "PUT",
        body: formData,
      });
