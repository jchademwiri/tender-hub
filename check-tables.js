...
async function checkDatabase() {
  try {
    ...
    if (tables.some(t => t.table_name === 'invitation')) {
      const columns = await sql`SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_name = 'invitation' ORDER BY ordinal_position`;
